...
const CreateCustomCodingSheetsEdit = (userGlobal) => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `http://ec2-15-206-82-121.ap-south-1.compute.amazonaws.com:8000/problem-sheets/details?sheetId=${sheetId}`
        );
        ...
        try {
          // alert("Hello World");
          const response = await fetch(`http://ec2-15-206-82-121.ap-south-1.compute.amazonaws.com:8000/user-details/profile-details/?ownerId=${sheetOwnerId}`);
          if (response.ok) {
  ...
  const handleExportProblemSheet = async () => {
    ...
    try {
      ...
      const response2 = await axios.post("http://ec2-15-206-82-121.ap-south-1.compute.amazonaws.com:8000/user-details/profile-update", userData, {
        withCredentials: true,
      });
      ...
      setTimeout(() => {
        if (response.status === 200) {
          ...
          window.open(
            `http://ec2-15-206-82-121.ap-south-1.compute.amazonaws.com:8000/create-problem-list/sheet/${sheetId}`,
            "_blank"
          );
        } else {
