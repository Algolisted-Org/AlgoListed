...
const create_resume = async (req, res) => {
    const { name, company, description, resume, linkedin, codingProfiles, medium, referral, mentorship } = req.body;

    try {
        const result = await All_Resumes.create({
            name, company, description, resume, linkedin, codingProfiles, medium, referral, mentorship
        })
